<template>
  <div
    :class="{'space_top': isSuccess || isFailed ? false : true}"
    class="forgot_pass_container w-50 d-flex flex-column justify-content-center align-items-center">
    <div class="forgot-form-container d-flex flex-column align-items-start">
      ...
      <div class="forgot-form bg-NTUC-7">
        <div class="fpg-logo">
        ...
        <div class="askAccount line-height-12 text-black font-400 mt--30 font__regular text-16">
          ...
          <a class="text-decoration-none cursor_pointer" @click="_goToLogin">
            {{ $t('btn.signin') }}
          </a>
        </div>
        <div class="credentials mt--30">
          ...
          <p v-if="!passwordError && !userIdError" class="
              forgot-slogan
              line-height-19
              mt--10
              font-400
        ...
        <b-form class="form-password mt--10">
          ...
          <b-form-group id="form-group-userId" class="mb-0"
            :class="{'color-box-forgot': this.form.Email != null ? true : false}"
          >
            <b-form-input class="form-input text-16" v-model="$v.form.UserId.$model" :state="validateState('UserId')"
              :placeholder="$t('forgetPass.input-userId')" :disabled="this.form.Email != null ? true : false" />
          </b-form-group>
          <b-form-group id="form-group-dob" class="mb-0 date_picker_forgotPass"
            :class="{'color-box-forgot': this.form.Email != null ? true : false}"
          >
            <DatetimePicker :placeholder="placeholder" v-model="$v.form.DateOfBirth.$model"
              :state="validateState('DateOfBirth')" :options="options"
        ...
        <b-form class="form-userId mt--10">
          ...
          <b-form-group id="form-group-email" class="mb-0"
            :class="{'color-form-userId': (this.form.UserId || this.form.DateOfBirth) != null ? true : false}"
          >
            <b-form-input class="form-input text-16" type="email" v-model="$v.form.Email.$model"
              :state="validateState('Email')" :placeholder="$t('forgetPass.input-email')"
        ...
        <b-button class="
            ...
            align-items-center
            justify-content-center
          " :disabled="(form.UserId || form.DateOfBirth || form.Email) == null" @click="onSubmit">
          {{ $t('btn.submit') }}
        </b-button>
        <b-button class="
            btn-submit
      ...
      if ((this.form.UserId || this.form.DateOfBirth) != null) {
        await this.fetchresetPassword({
          ...
          a1: {
            ...
            DOB: this.form.DateOfBirth != null ? this._formatTime(this.form.DateOfBirth) : "Invalid Date",
            SendEmail: '1'
          }
        });
